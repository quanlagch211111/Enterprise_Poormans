...
const Account = () => {
  ...
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    // Kiểm tra nếu name bắt đầu với "additionalInfo"
    if (name.startsWith("additionalInfo")) {
      const field = name.split(".")[1]; // Lấy trường trong `additionalInfo`
      setSelectedAccount({
        ...selectedAccount,
        additionalInfo: {
      ...
      setSelectedAccount({
        ...selectedAccount,
        [name]: value, // Cập nhật các trường ngoài `additionalInfo`
      });
    }
};
  ...
  const handleCreateAccount = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3001/api/users/signup', requestData);
  ...
  const handleUpdateAccount = async () => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:3001/api/users/update/${selectedAccount._id}`,
        selectedAccount,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      ...
      <div className="modal-body">
        {selectedAccount && (
          <>
            ...
            <div className="form-group mb-3">
              <label htmlFor="editVerified">Xác minh</label>
              <select
                className="form-control"
                id="editVerified"
                name="status"
                value={selectedAccount.status || false}
                onChange={handleInputChange}
              >
